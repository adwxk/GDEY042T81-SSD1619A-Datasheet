...
| `powerOff()` | **必须保留**（发送0x02关闭驱动电压，~81ms）。跳过会导致断电花屏 |
...
| 操作 | Mode Byte | T1 | BUSY时间 | PowerOff | 总时间 | 闪烁 |
| --- | --- | ---: | ---: | ---: | ---: | --- |
| OTP 标准全刷（开机） | 0xF7 | - | 3175ms | - | ~3.3s | 多次 |
| 自定义LUT快速翻页 | 0xC5 | 10 | 237ms | 81ms | **~318ms** | 不闪 |
| 自定义LUT清残影刷 | 0xC5 | 31 | 808ms | 81ms | **~890ms** | 不闪 |

> PowerOff（0x02）必须在每次刷新后执行，关闭驱动电压源。跳过会导致断电花屏（竖线+噪点）。

### 优化历程

| 阶段 | 翻页时间 | 优化点 |
| --- | ---: | --- |
| 原始LUT(T1=31) | 889ms | 基准 |
| 减少帧数(T1=10) | 318ms | 快2.8倍，轻微残影 |
| 清残影全刷替代OTP | 890ms | 替代3175ms的OTP，快3.6倍 |
| **当前最终方案** | **318ms** | T1=10 + PowerOff保留 + 每30页清残影 |
...
| 翻页有81ms额外延迟 | PowerOff等待 | **必须保留**，跳过会导致断电花屏 |
| 断电花屏（竖线+噪点） | 断电时IC逻辑电源在驱动电压前降到不安全电压 | 保留PowerOff关闭驱动电压源 |
